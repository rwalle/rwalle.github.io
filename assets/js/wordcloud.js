...
function generateCloud() {
    ...
    fontsizeBase = Math.sqrt(width / 800) * 16;
    for (let i = 0; i < words.length; i++) {
        words[i].size = Math.sqrt(words[i].size0) * fontsizeBase + (Math.random() - 0.5) * 4;
    }
    d3.layout.cloud()
      .size([width, height])
      .words(words)
